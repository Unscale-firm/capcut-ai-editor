"""Revert side angle to ORIGINAL framing (scale 1.0, transform 0) and rebuild the 6 angle-transition
clips with the original side framing (so held side doesn't pop). Keeps dip/punch effects.
...
SCROP='scale=1080:1920,fps=30'   # ORIGINAL side framing (no reframe)
...
def build(cut,from_tag,to_tag,effect):
    PRE,POST=(6,6) if effect=='dip' else (4,9)
    NF=PRE+POST; ci=PRE
    ...
    mp4=DL+f'0616_atr_{str(cut).replace(".","")}_o.mp4'
...
for cut,a,b,eff in CUTS:
    mp4,pt=build(cut,a,b,eff); placements.append((mp4,pt)); print(f'rebuilt {eff} @ {cut} (orig side) -> {pt}')

shutil.copy2(PROJ,PROJ+'.before-revertside')
M=d['materials']; vids={m['id']:m for m in M['videos']}
# 1) revert side framing
ns=0
for t in d['tracks']:
    if t['type']!='video': continue
    for s in t['segments']:
        if 'C9138' in vids.get(s['material_id'],{}).get('path',''):
            c=s.setdefault('clip',{}); c['scale']={'x':1.0,'y':1.0}; c['transform']={'x':0.0,'y':0.0}; ns+=1
print('reverted side segments to 1.0:',ns)
# 2) remove old atr tracks + materials
old=set(); kept=[]
for t in d['tracks']:
    drop=False
    if t['type']=='video':
        for s in t['segments']:
            if '0616_atr_' in vids.get(s['material_id'],{}).get('path','') and not vids.get(s['material_id'],{}).get('path','').endswith('_o.mp4'):
                drop=True; old.add(s['material_id'])
    if not drop: kept.append(t)
d['tracks']=kept; M['videos']=[m for m in M['videos'] if m['id'] not in old]
print('removed old atr tracks:',len(old))
# 3) place rebuilt clips
idmap={}
for k,v in M.items():
...
print('done — side reverted, 6 transitions rebuilt with original side framing | SAVED')
